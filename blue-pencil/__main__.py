...
def main():
    ...
    if (args.filename.endswith(".pdf") and args.pdf2text):
        processPDF2Text(args.filename, extra_patterns,args.replace,args.outputFile)

    elif (args.filename.endswith(".pdf")):
        processPDF(args.filename, extra_patterns,args.replace,args.outputFile)
        ...
        processTextF(args.filename, extra_patterns,args.replace,args.outputFile)
...
def processTextF(filename,extra_patterns,replace,outputF):
    text =  open(filename).read()
    ents=finder.find_entities(text,extra_patterns)
    output = find_and_censor.blue_markerText(ents,text,replace)
    open(outputF+".txt","w").write(output)


def processPDF2Text(filename,extra_patterns,replace,outputF):
    txtfiles = disasemble.pdf_to_text(filename,"pdfText/text")
    output=""
    counter=0
    for txt in txtfiles:
        text =  open(txt).read()
        result= finder.find_entities(text,extra_patterns)
        output+= find_and_censor.blue_markerText(result,text,replace)
        counter+=1
    open(outputF+".txt","w").write(output)

def processPDF(filename,extra_patterns,replace,outputF):
